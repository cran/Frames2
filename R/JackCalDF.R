...
#' 
#' @usage JackCalDF(ysA, ysB, piA, piB, domainsA, domainsB, N_A = NULL, N_B = NULL, 
...
#' \eqn{\hat{Y}_{c}^{B*}(lj)= \hat{Y}_{c}+\sqrt{1-\overline{\pi}_B} (\hat{Y}_{c}^{B}(lj) -\hat{Y}_{c})}, where \eqn{\overline{\pi}_A = \sum_{i \in s_A}\pi_{iA}/nA} and \eqn{\overline{\pi}_B = \sum_{j \in s_A}\pi_{jB}/nB}
...
#' @export
JackCalDF = function (ysA, ysB, piA, piB, domainsA, domainsB, N_A = NULL, N_B = NULL, N_ab = NULL, xsAFrameA = NULL, xsBFrameA = NULL, xsAFrameB = NULL, xsBFrameB = NULL, xsT = NULL, XA = NULL, XB = NULL, X = NULL, met = "linear", conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- CalDF(ysA, ysB, piA, piB, domainsA, domainsB, N_A, N_B, N_ab, xsAFrameA, xsBFrameA, xsAFrameB, xsBFrameB, xsT, XA, XB, X, met)
	...
	if (sdA == "str"){
		...
		for (i in 1:nstrataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(piA))))
					YcA[k,] <- CalDF(ysA[-j,], ysB, piA[-j,-j], piB, domainsA[-j], domainsB, N_A, N_B, N_ab, xsAFrameA[-j,], xsBFrameA, xsAFrameB[-j,], xsBFrameB, xsT[-j,], XA, XB, X, met)[[2]][1,]
				else
					YcA[k,] <- CalDF(ysA[-j,], ysB, piA[-j], piB, domainsA[-j], domainsB, N_A, N_B, N_ab, xsAFrameA[-j,], xsBFrameA, xsAFrameB[-j,], xsBFrameB, xsT[-j,], XA, XB, X, met)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdA == "clu"){
			...
			for (i in 1:nclustersA){
				if (!is.null(dim(drop(piA))))
					YcA[i,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]],clusA %in% clustersA[-clustersA[i]]], piB, domainsA[clusA %in% clustersA[-clustersA[i]]], domainsB, N_A, N_B, N_ab, xsAFrameA[clusA %in% clustersA[-clustersA[i]],], xsBFrameA, xsAFrameB[clusA %in% clustersA[-clustersA[i]],], xsBFrameB, xsT[clusA %in% clustersA[-clustersA[i]],], XA, XB, X, met)[[2]][1,]
				else
					YcA[i,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]]], piB, domainsA[clusA %in% clustersA[-clustersA[i]]], domainsB, N_A, N_B, N_ab, xsAFrameA[clusA %in% clustersA[-clustersA[i]],], xsBFrameA, xsAFrameB[clusA %in% clustersA[-clustersA[i]],], xsBFrameB, xsT[clusA %in% clustersA[-clustersA[i]],], XA, XB, X, met)[[2]][1,]
			}
			...
			if (sdA == "strclu"){
				...
				for (i in 1:nstrataA){
					...
					for (j in 1:nclustersA){
						...
						if (!is.null(dim(drop(piA))))
							YcA[k,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]],clusA %in% clustersA[-clustersA[j]]], piB, domainsA[clusA %in% clustersA[-clustersA[j]]], domainsB, N_A, N_B, N_ab, xsAFrameA[clusA %in% clustersA[-clustersA[j]],], xsBFrameA, xsAFrameB[clusA %in% clustersA[-clustersA[j]],], xsBFrameB, xsT[clusA %in% clustersA[-clustersA[j]],], XA, XB, X, met)[[2]][1,]
						else
							YcA[k,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]]], piB, domainsA[clusA %in% clustersA[-clustersA[j]]], domainsB, N_A, N_B, N_ab, xsAFrameA[clusA %in% clustersA[-clustersA[j]],], xsBFrameA, xsAFrameB[clusA %in% clustersA[-clustersA[j]],], xsBFrameB, xsT[clusA %in% clustersA[-clustersA[j]],], XA, XB, X, met)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nA){
					if (!is.null(dim(drop(piA))))
						YcA[i,] <- CalDF(ysA[-i,], ysB, piA[-i,-i], piB, domainsA[-i], domainsB, N_A, N_B, N_ab, xsAFrameA[-i,], xsBFrameA, xsAFrameB[-i,], xsBFrameB, xsT[-i,], XA, XB, X, met)[[2]][1,]
					else
						YcA[i,] <- CalDF(ysA[-i,], ysB, piA[-i], piB, domainsA[-i], domainsB, N_A, N_B, N_ab, xsAFrameA[-i,], xsBFrameA, xsAFrameB[-i,], xsBFrameB, xsT[-i,], XA, XB, X, met)[[2]][1,]		
				}
	...
	if (sdB == "str"){
		...
		for (i in 1:nstrataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(piB))))
					YcB[k,] <- CalDF(ysA, ysB[-j,], piA, piB[-j,-j], domainsA, domainsB[-j], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[-j,], xsAFrameB, xsBFrameB[-j,], xsT[-(nA + j),], XA, XB, X, met)[[2]][1,]		
				else
					YcB[k,] <- CalDF(ysA, ysB[-j,], piA, piB[-j], domainsA, domainsB[-j], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[-j,], xsAFrameB, xsBFrameB[-j,], xsT[-(nA + j),], XA, XB, X, met)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdB == "clu"){
			...
			for (i in 1:nclustersB){
				if (!is.null(dim(drop(piB))))
					YcB[i,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]],clusB %in% clustersB[-clustersB[i]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[i]]], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[clusB %in% clustersB[-clustersB[i]],], xsAFrameB, xsBFrameB[clusB %in% clustersB[-clustersB[i]],], xsT[clusB %in% clustersB[-clustersB[i]],], XA, XB, X, met)[[2]][1,]
				else
					YcB[i,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[i]]], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[clusB %in% clustersB[-clustersB[i]],], xsAFrameB, xsBFrameB[clusB %in% clustersB[-clustersB[i]],], xsT[clusB %in% clustersB[-clustersB[i]],], XA, XB, X, met)[[2]][1,]
			}
			...
			if(sdB == "strclu"){
				...
				for (i in 1:nstrataB){
					...
					for (j in 1:nclustersB){
						...
						if (!is.null(dim(drop(piB))))
							YcB[k,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]],clusB %in% clustersB[-clustersB[j]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[j]]], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[clusB %in% clustersB[-clustersB[j]],], xsAFrameB, xsBFrameB[clusB %in% clustersB[-clustersB[j]],], xsT[clusB %in% clustersB[-clustersB[j]],], XA, XB, X, met)[[2]][1,]
						else
							YcB[k,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[j]]], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[clusB %in% clustersB[-clustersB[j]],], xsAFrameB, xsBFrameB[clusB %in% clustersB[-clustersB[j]],], xsT[clusB %in% clustersB[-clustersB[j]],], XA, XB, X, met)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nB){
					...
					if (!is.null(dim(drop(piB))))
						YcB[i,] <- CalDF(ysA, ysB[-i,], piA, piB[-i,-i], domainsA, domainsB[-i], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[-i,], xsAFrameB, xsBFrameB[-i,], xsT[-(nA + i),], XA, XB, X, met)[[2]][1,]
					else
						YcB[i,] <- CalDF(ysA, ysB[-i,], piA, piB[-i], domainsA, domainsB[-i], N_A, N_B, N_ab, xsAFrameA, xsBFrameA[-i,], xsAFrameB, xsBFrameB[-i,], xsT[-(nA + i),], XA, XB, X, met)[[2]][1,]
				}
