...
#' conf_level, sdA = "srs", sdB = "srs", nhA = NULL, NhA = NULL, nhB = NULL, 
#' NhB = NULL, fcpA = FALSE, fcpB = FALSE)
...
#' JackCalDF(dataA$Clothing, dataB$Clothing, dataA$ProbA, dataB$ProbB, 
#' dataA$Domain, dataB$Domain, N_A = 1735, N_B = 1191, N_ab = 601, conf_level = 0.95,
#' sdA = "str", sdB = "srs", nhA = c(15, 20, 15, 20, 15, 20))
#' 
#' #Finally, let consider a finite population correction factor in both frames.
#' JackCalDF(dataA$Clothing, dataB$Clothing, dataA$ProbA, dataB$ProbB, 
#' dataA$Domain, dataB$Domain, N_A = 1735, N_B = 1191, N_ab = 601, conf_level = 0.95,
#' sdB = "srs", nhA = c(15, 20, 15, 20, 15, 20), NhA = c(727, 375, 113, 186, 115, 219),
#' sdA = "str", fcpA = TRUE, fcpB = TRUE)
#' @export
JackCalDF = function (ysA, ysB, pi_A, pi_B, domains_A, domains_B, N_A = NULL, N_B = NULL, N_ab = NULL, XsAFrameA = NULL, XsBFrameA = NULL, XsAFrameB = NULL, XsBFrameB = NULL, XsT = NULL, XA = NULL, XB = NULL, X = NULL, met = "linear", conf_level, sdA = "srs", sdB = "srs", nhA = NULL, NhA = NULL, nhB = NULL, NhB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	if (sdA == "str"){
		...
		for (i in 1:strataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(pi_A))))
					YcA[k,] <- CalDF(ysA[-j,], ysB, pi_A[-j,-j], pi_B, domains_A[-j], domains_B, N_A, N_B, N_ab, XsAFrameA[-j,], XsBFrameA, XsAFrameB[-j,], XsBFrameB, XsT[-j,], XA, XB, X, met)[1,]
				else
					YcA[k,] <- CalDF(ysA[-j,], ysB, pi_A[-j], pi_B, domains_A[-j], domains_B, N_A, N_B, N_ab, XsAFrameA[-j,], XsBFrameA, XsAFrameB[-j,], XsBFrameB, XsT[-j,], XA, XB, X, met)[1,]
				k <- k + 1
			}
		...
		for (i in 1:n_A){
			...
			if (!is.null(dim(drop(pi_A))))
				YcA[i,] <- CalDF(ysA[-i,], ysB, pi_A[-i,-i], pi_B, domains_A[-i], domains_B, N_A, N_B, N_ab, XsAFrameA[-i,], XsBFrameA, XsAFrameB[-i,], XsBFrameB, XsT[-i,], XA, XB, X, met)[1,]
			else
				YcA[i,] <- CalDF(ysA[-i,], ysB, pi_A[-i], pi_B, domains_A[-i], domains_B, N_A, N_B, N_ab, XsAFrameA[-i,], XsBFrameA, XsAFrameB[-i,], XsBFrameB, XsT[-i,], XA, XB, X, met)[1,]		
		}
	...
	if (sdB == "str"){
		...
		for (i in 1:strataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(pi_B))))
					YcB[k,] <- CalDF(ysA, ysB[-j,], pi_A, pi_B[-j,-j], domains_A, domains_B[-j], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-j,], XsAFrameB, XsBFrameB[-j,], XsT[-j,], XA, XB, X, met)[1,]		
				else 
					YcB[k,] <- CalDF(ysA, ysB[-j,], pi_A, pi_B[-j], domains_A, domains_B[-j], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-j,], XsAFrameB, XsBFrameB[-j,], XsT[-j,], XA, XB, X, met)[1,]
				k <- k + 1
			}
		...
		for (i in 1:n_B){
	        	...
	        	if (!is.null(dim(drop(pi_B))))
				YcB[i,] <- CalDF(ysA, ysB[-i,], pi_A, pi_B[-i,-i], domains_A, domains_B[-i], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-i,], XsAFrameB, XsBFrameB[-i,], XsT[-i,], XA, XB, X, met)[1,]
			else
				YcB[i,] <- CalDF(ysA, ysB[-i,], pi_A, pi_B[-i], domains_A, domains_B[-i], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-i,], XsAFrameB, XsBFrameB[-i,], XsT[-i,], XA, XB, X, met)[1,]
		}
	...
	results[2,] <- estimation[1,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_CalDF)
	results[3,] <- estimation[1,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_CalDF)
	results[4,] <- estimation[2,]
	results[5,] <- estimation[2,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_CalDF)
	results[6,] <- estimation[2,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_CalDF)
