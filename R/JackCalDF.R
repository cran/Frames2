#' @name JackCalDF
#' @aliases JackCalDF
#' @title Confidence intervals for dual frame calibration estimator based on jackknife method
#' 
#' @description Calculates confidence intervals for dual frame calibration estimator using jackknife procedure
#' 
#' @usage JackCalDF(ysA, ysB, pi_A, pi_B, domains_A, domains_B, N_A = NULL, N_B = NULL, 
...
#' \eqn{\hat{Y}_{c}^{B*}(lj)= \hat{Y}_{c}+\sqrt{1-\overline{\pi}_B} (\hat{Y}_{c}^{B}(lj) -\hat{Y}_{c})}, where \eqn{\overline{\pi}_A = \sum_{i \in s_A}\pi_{iA}/n_A} and \eqn{\overline{\pi}_B = \sum_{j \in s_A}\pi_{jB}/n_B}
...
#' JackCalDF(DatA$Clo, DatB$Clo, DatA$ProbA, DatB$ProbB, 
#' DatA$Domain, DatB$Domain, N_A = 1735, N_B = 1191, N_ab = 601, conf_level = 0.95,
#' sdA = "str", sdB = "srs", strA = DatA$Stratum, NhA = c(727, 375, 113, 186, 115, 219),
#' fcpA = TRUE, fcpB = TRUE)
#' @export
JackCalDF = function (ysA, ysB, pi_A, pi_B, domains_A, domains_B, N_A = NULL, N_B = NULL, N_ab = NULL, XsAFrameA = NULL, XsBFrameA = NULL, XsAFrameB = NULL, XsBFrameB = NULL, XsT = NULL, XA = NULL, XB = NULL, X = NULL, met = "linear", conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, NhA = NULL, NhB = NULL, NclushA = NULL, NclushB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- CalDF(ysA, ysB, pi_A, pi_B, domains_A, domains_B, N_A, N_B, N_ab, XsAFrameA, XsBFrameA, XsAFrameB, XsBFrameB, XsT, XA, XB, X, met)
	...
	if (sdA == "str"){
		...
		for (i in 1:nstrataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(pi_A))))
					YcA[k,] <- CalDF(ysA[-j,], ysB, pi_A[-j,-j], pi_B, domains_A[-j], domains_B, N_A, N_B, N_ab, XsAFrameA[-j,], XsBFrameA, XsAFrameB[-j,], XsBFrameB, XsT[-j,], XA, XB, X, met)[[2]][1,]
				else
					YcA[k,] <- CalDF(ysA[-j,], ysB, pi_A[-j], pi_B, domains_A[-j], domains_B, N_A, N_B, N_ab, XsAFrameA[-j,], XsBFrameA, XsAFrameB[-j,], XsBFrameB, XsT[-j,], XA, XB, X, met)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdA == "clu"){
			...
			for (i in 1:nclustersA){
				if (!is.null(dim(drop(pi_A))))
					YcA[i,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pi_A[clusA %in% clustersA[-clustersA[i]],clusA %in% clustersA[-clustersA[i]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[i]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[i]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[i]],], XA, XB, X, met)[[2]][1,]
				else
					YcA[i,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pi_A[clusA %in% clustersA[-clustersA[i]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[i]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[i]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[i]],], XA, XB, X, met)[[2]][1,]
			}
			...
			if (sdA == "strclu"){
				...
				for (i in 1:nstrataA){
					...
					for (j in 1:nclustersA){
						...
						if (!is.null(dim(drop(pi_A))))
							YcA[k,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pi_A[clusA %in% clustersA[-clustersA[j]],clusA %in% clustersA[-clustersA[j]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[j]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[j]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[j]],], XA, XB, X, met)[[2]][1,]
						else
							YcA[k,] <- CalDF(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pi_A[clusA %in% clustersA[-clustersA[j]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[j]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[j]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[j]],], XA, XB, X, met)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:n_A){
					if (!is.null(dim(drop(pi_A))))
						YcA[i,] <- CalDF(ysA[-i,], ysB, pi_A[-i,-i], pi_B, domains_A[-i], domains_B, N_A, N_B, N_ab, XsAFrameA[-i,], XsBFrameA, XsAFrameB[-i,], XsBFrameB, XsT[-i,], XA, XB, X, met)[[2]][1,]
					else
						YcA[i,] <- CalDF(ysA[-i,], ysB, pi_A[-i], pi_B, domains_A[-i], domains_B, N_A, N_B, N_ab, XsAFrameA[-i,], XsBFrameA, XsAFrameB[-i,], XsBFrameB, XsT[-i,], XA, XB, X, met)[[2]][1,]		
				}
	...
	if (sdB == "str"){
		...
		for (i in 1:strataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(pi_B))))
					YcB[k,] <- CalDF(ysA, ysB[-j,], pi_A, pi_B[-j,-j], domains_A, domains_B[-j], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-j,], XsAFrameB, XsBFrameB[-j,], XsT[-j,], XA, XB, X, met)[[2]][1,]		
				else
					YcB[k,] <- CalDF(ysA, ysB[-j,], pi_A, pi_B[-j], domains_A, domains_B[-j], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-j,], XsAFrameB, XsBFrameB[-j,], XsT[-j,], XA, XB, X, met)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdB == "clu"){
			...
			for (i in 1:nclustersB){
				if (!is.null(dim(drop(pi_B))))
					YcB[i,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[i]],clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[i]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[i]],], XsT[clusB %in% clustersB[-clustersB[i]],], XA, XB, X, met)[[2]][1,]
				else
					YcB[i,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[i]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[i]],], XsT[clusB %in% clustersB[-clustersB[i]],], XA, XB, X, met)[[2]][1,]
			}
			...
			if(sdB == "strclu"){
				...
				for (i in 1:nstrataB){
					...
					for (j in 1:nclustersB){
						...
						if (!is.null(dim(drop(pi_B))))
							YcB[k,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[j]],clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[j]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[j]],], XsT[clusB %in% clustersB[-clustersB[j]],], XA, XB, X, met)[[2]][1,]
						else
							YcB[k,] <- CalDF(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[j]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[j]],], XsT[clusB %in% clustersB[-clustersB[j]],], XA, XB, X, met)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:n_B){
					...
					if (!is.null(dim(drop(pi_B))))
						YcB[i,] <- CalDF(ysA, ysB[-i,], pi_A, pi_B[-i,-i], domains_A, domains_B[-i], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-i,], XsAFrameB, XsBFrameB[-i,], XsT[-i,], XA, XB, X, met)[[2]][1,]
					else
						YcB[i,] <- CalDF(ysA, ysB[-i,], pi_A, pi_B[-i], domains_A, domains_B[-i], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-i,], XsAFrameB, XsBFrameB[-i,], XsT[-i,], XA, XB, X, met)[[2]][1,]
				}
	...
	VJack_Yhat_CalDF <- vjA + vjB

	results[1,] <- estimation[[2]][1,]
	results[2,] <- estimation[[2]][1,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_CalDF)
	results[3,] <- estimation[[2]][1,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_CalDF)
	results[4,] <- estimation[[2]][2,]
	results[5,] <- estimation[[2]][2,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_CalDF)
	results[6,] <- estimation[[2]][2,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_CalDF)
