...
#' 
#' @usage JackHartley(ysA, ysB, piA, piB, domainsA, domainsB, conf_level, sdA = "srs", 
...
#' fcpB = TRUE)
#' @export
JackHartley = function (ysA, ysB, piA, piB, domainsA, domainsB, conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- Hartley(ysA, ysB, piA, piB, domainsA, domainsB)
	...
	if (sdA == "str"){
		...
		for (i in 1:nstrataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(piA))))
					YcA[k,] <- Hartley(ysA[-j,], ysB, piA[-j,-j], piB, domainsA[-j], domainsB)[[2]][1,]
				else
					YcA[k,] <- Hartley(ysA[-j,], ysB, piA[-j], piB, domainsA[-j], domainsB)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdA == "clu"){
			...
			for (i in 1:nclustersA){
				if (!is.null(dim(drop(piA))))
					YcA[i,] <- Hartley(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]],clusA %in% clustersA[-clustersA[i]]], piB, domainsA[clusA %in% clustersA[-clustersA[i]]], domainsB)[[2]][1,]
				else
					YcA[i,] <- Hartley(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, piA[clusA %in% clustersA[-clustersA[i]]], piB, domainsA[clusA %in% clustersA[-clustersA[i]]], domainsB)[[2]][1,]
			}
			...
			if (sdA == "strclu"){
				...
				for (i in 1:nstrataA){
					...
					for (j in 1:nclustersA){
						...
						if (!is.null(dim(drop(piA))))
							YcA[k,] <- Hartley(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]],clusA %in% clustersA[-clustersA[j]]], piB, domainsA[clusA %in% clustersA[-clustersA[j]]], domainsB)[[2]][1,]
						else
							YcA[k,] <- Hartley(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, piA[clusA %in% clustersA[-clustersA[j]]], piB, domainsA[clusA %in% clustersA[-clustersA[j]]], domainsB)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nA){
					if (!is.null(dim(drop(piA))))
						YcA[i,] <- Hartley(ysA[-i,], ysB, piA[-i,-i], piB, domainsA[-i], domainsB)[[2]][1,]
					else
						YcA[i,] <- Hartley(ysA[-i,], ysB, piA[-i], piB, domainsA[-i], domainsB)[[2]][1,]
				}
	...
	if (sdB == "str"){
		...
		for (i in 1:nstrataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(piB))))
					YcB[k,] <- Hartley(ysA, ysB[-j,], piA, piB[-j,-j], domainsA, domainsB[-j])[[2]][1,]
				else
					YcB[k,] <- Hartley(ysA, ysB[-j,], piA, piB[-j], domainsA, domainsB[-j])[[2]][1,]
				k <- k + 1
			}
		...
		if (sdB == "clu"){
			...
			for (i in 1:nclustersB){
				if (!is.null(dim(drop(piB))))
					YcB[i,] <- Hartley(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]],clusB %in% clustersB[-clustersB[i]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
				else
					YcB[i,] <- Hartley(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], piA, piB[clusB %in% clustersB[-clustersB[i]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
			}
			...
			if(sdB == "strclu"){
				...
				for (i in 1:nstrataB){
					...
					for (j in 1:nclustersB){
						...
						if (!is.null(dim(drop(piB))))
							YcB[k,] <- Hartley(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]],clusB %in% clustersB[-clustersB[j]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						else
							YcB[k,] <- Hartley(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], piA, piB[clusB %in% clustersB[-clustersB[j]]], domainsA, domainsB[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:nB){
					...
					if (!is.null(dim(drop(piB))))
						YcB[i,] <- Hartley(ysA, ysB[-i,], piA, piB[-i,-i], domainsA, domainsB[-i])[[2]][1,]
					else
						YcB[i,] <- Hartley(ysA, ysB[-i,], piA, piB[-i], domainsA, domainsB[-i])[[2]][1,]
				}
