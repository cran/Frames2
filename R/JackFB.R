#' @name JackFB
#' @aliases JackFB
#' @title Confidence intervals for Fuller-Burmeister estimator based on jackknife method
#' 
#' @description Calculates confidence intervals for Fuller-Burmeister estimator using jackknife procedure
#' 
#' @usage JackFB(ysA, ysB, pi_A, pi_B, domains_A, domains_B, conf_level, sdA = "srs", 
...
#' @export
JackFB = function (ysA, ysB, pi_A, pi_B, domains_A, domains_B, conf_level, sdA = "srs", sdB = "srs", strA = NULL, strB = NULL, clusA = NULL, clusB = NULL, NhA = NULL, NhB = NULL, NclushA = NULL, NclushB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	estimation <- FB(ysA, ysB, pi_A, pi_B, domains_A, domains_B)
	...
	if (sdA == "str"){
		...
		for (i in 1:nstrataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(pi_A))))
					YcA[k,] <- FB(ysA[-j,], ysB, pi_A[-j,-j], pi_B, domains_A[-j], domains_B)[[2]][1,]
				else
					YcA[k,] <- FB(ysA[-j,], ysB, pi_A[-j], pi_B, domains_A[-j], domains_B)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdA == "clu"){
			...
			for (i in 1:nclustersA){
				if (!is.null(dim(drop(pi_A))))
					YcA[i,] <- FB(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pi_A[clusA %in% clustersA[-clustersA[i]],clusA %in% clustersA[-clustersA[i]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B)[[2]][1,]
				else
					YcA[i,] <- FB(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pi_A[clusA %in% clustersA[-clustersA[i]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B)[[2]][1,]
			}
			...
			if (sdA == "strclu"){
				...
				for (i in 1:nstrataA){
					...
					for (j in 1:nclustersA){
						...
						if (!is.null(dim(drop(pi_A))))
							YcA[k,] <- FB(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pi_A[clusA %in% clustersA[-clustersA[j]],clusA %in% clustersA[-clustersA[j]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B)[[2]][1,]
						else
							YcA[k,] <- FB(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pi_A[clusA %in% clustersA[-clustersA[j]]], pi_B, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:n_A){
					if (!is.null(dim(drop(pi_A))))
						YcA[i,] <- FB(ysA[-i,], ysB, pi_A[-i,-i], pi_B, domains_A[-i], domains_B)[[2]][1,]
					else
						YcA[i,] <- FB(ysA[-i,], ysB, pi_A[-i], pi_B, domains_A[-i], domains_B)[[2]][1,]
				}
	...
	if (sdB == "str"){
		...
		for (i in 1:strataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(pi_B))))
					YcB[k,] <- FB(ysA, ysB[-j,], pi_A, pi_B[-j,-j], domains_A, domains_B[-j])[[2]][1,]
				else
					YcB[k,] <- FB(ysA, ysB[-j,], pi_A, pi_B[-j], domains_A, domains_B[-j])[[2]][1,]
				k <- k + 1
			}
		...
		if (sdB == "clu"){
			...
			for (i in 1:nclustersB){
				if (!is.null(dim(drop(pi_B))))
					YcB[i,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[i]],clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
				else
					YcB[i,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]])[[2]][1,]
			}
			...
			if(sdB == "strclu"){
				...
				for (i in 1:nstrataB){
					...
					for (j in 1:nclustersB){
						...
						if (!is.null(dim(drop(pi_B))))
							YcB[k,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[j]],clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						else
							YcB[k,] <- FB(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]])[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:n_B){
					...
					if (!is.null(dim(drop(pi_B))))
						YcB[i,] <- FB(ysA, ysB[-i,], pi_A, pi_B[-i,-i], domains_A, domains_B[-i])[[2]][1,]
					else
						YcB[i,] <- FB(ysA, ysB[-i,], pi_A, pi_B[-i], domains_A, domains_B[-i])[[2]][1,]
				}
	...
	VJack_Yhat_FB <- vjA + vjB

	results[1,] <- estimation[[2]][1,]
	results[2,] <- estimation[[2]][1,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_FB)
	results[3,] <- estimation[[2]][1,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_FB)
	results[4,] <- estimation[[2]][2,]
	results[5,] <- estimation[[2]][2,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_FB)
	results[6,] <- estimation[[2]][2,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_FB)
