...
#' JackFB(dataA$Clothing, dataB$Clothing, dataA$ProbA, dataB$ProbB, dataA$Domain, 
...
#' @export
JackFB = function (ysA, ysB, pi_A, pi_B, domains_A, domains_B, conf_level, sdA = "srs", sdB = "srs", nhA = NULL, NhA = NULL, nhB = NULL, NhB = NULL, fcpA = FALSE, fcpB = FALSE){
	...
	if (sdA == "str"){
		...
		for (i in 1:strataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(pi_A))))
					YcA[k,] <- FB(ysA[-j,], ysB, pi_A[-j,-j], pi_B, domains_A[-j], domains_B)[1,]
				else
					YcA[k,] <- FB(ysA[-j,], ysB, pi_A[-j], pi_B, domains_A[-j], domains_B)[1,]
				k <- k + 1
			}
		...
		for (i in 1:n_A){
			if (!is.null(dim(drop(pi_A))))
				YcA[i,] <- FB(ysA[-i,], ysB, pi_A[-i,-i], pi_B, domains_A[-i], domains_B)[1,]
			else
				YcA[i,] <- FB(ysA[-i,], ysB, pi_A[-i], pi_B, domains_A[-i], domains_B)[1,]
		}
	...
	if (sdB == "str"){
		...
		for (i in 1:strataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(pi_B))))
					YcB[k,] <- FB(ysA, ysB[-j,], pi_A, pi_B[-j,-j], domains_A, domains_B[-j])[1,]
				else
					YcB[k,] <- FB(ysA, ysB[-j,], pi_A, pi_B[-j], domains_A, domains_B[-j])[1,]
				k <- k + 1
			}
		...
		for (i in 1:n_B){
			if (!is.null(dim(drop(pi_B))))
				YcB[i,] <- FB(ysA, ysB[-i,], pi_A, pi_B[-i,-i], domains_A, domains_B[-i])[1,]
			else
				YcB[i,] <- FB(ysA, ysB[-i,], pi_A, pi_B[-i], domains_A, domains_B[-i])[1,]
		}
	...
	results[2,] <- estimation[1,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_FB)
	results[3,] <- estimation[1,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(VJack_Yhat_FB)
	results[4,] <- estimation[2,]
	results[5,] <- estimation[2,] + qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_FB)
	results[6,] <- estimation[2,] - qnorm(1 - (1 - conf_level) / 2) * sqrt(1/size_estimation^2 * VJack_Yhat_FB)
