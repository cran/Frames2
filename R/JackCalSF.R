...
#' strB = NULL, clusA = NULL, clusB = NULL, NhA = NULL, NhB = NULL, NclushA = NULL,
#' NclushB = NULL, fcpA = FALSE, fcpB = FALSE)
	...
	if (sdA == "str"){
		...
		for (i in 1:nstrataA){
			...
			for (j in (cnhA[i]+1):cnhA[i+1]){
				...
				if (!is.null(dim(drop(pi_A))))
					YcA[k,] <- CalSF(ysA[-j,], ysB, pi_A[-j,-j], pi_B, pik_ab_B[-j], pik_ba_A, domains_A[-j], domains_B, N_A, N_B, N_ab, XsAFrameA[-j,], XsBFrameA, XsAFrameB[-j,], XsBFrameB, XsT[-j,], XA, XB, X, met)[[2]][1,]
				else
					YcA[k,] <- CalSF(ysA[-j,], ysB, pi_A[-j], pi_B, pik_ab_B[-j], pik_ba_A, domains_A[-j], domains_B, N_A, N_B, N_ab, XsAFrameA[-j,], XsBFrameA, XsAFrameB[-j,], XsBFrameB, XsT[-j,], XA, XB, X, met)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdA == "clu"){
			...
			for (i in 1:nclustersA){
				if (!is.null(dim(drop(pi_A))))
					YcA[i,] <- CalSF(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pi_A[clusA %in% clustersA[-clustersA[i]],clusA %in% clustersA[-clustersA[i]]], pi_B, pik_ab_B[clusA %in% clustersA[-clustersA[i]]], pik_ba_A, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[i]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[i]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[i]],], XA, XB, X, met)[[2]][1,]
				else
					YcA[i,] <- CalSF(ysA[clusA %in% clustersA[-clustersA[i]],], ysB, pi_A[clusA %in% clustersA[-clustersA[i]]], pi_B, pik_ab_B[clusA %in% clustersA[-clustersA[i]]], pik_ba_A, domains_A[clusA %in% clustersA[-clustersA[i]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[i]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[i]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[i]],], XA, XB, X, met)[[2]][1,]
			}
			...
			if (sdA == "strclu"){
				...
				for (i in 1:nstrataA){
					...
					for (j in 1:nclustersA){
						...
						if (!is.null(dim(drop(pi_A))))
							...
							YcA[k,] <- CalSF(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pi_A[clusA %in% clustersA[-clustersA[j]],clusA %in% clustersA[-clustersA[j]]], pi_B, pik_ab_B[clusA %in% clustersA[-clustersA[j]]], pik_ba_A, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[j]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[j]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[j]],], XA, XB, X, met)[[2]][1,]
						else
							YcA[k,] <- CalSF(ysA[clusA %in% clustersA[-clustersA[j]],], ysB, pi_A[clusA %in% clustersA[-clustersA[j]]], pi_B, pik_ab_B[clusA %in% clustersA[-clustersA[j]]], pik_ba_A, domains_A[clusA %in% clustersA[-clustersA[j]]], domains_B, N_A, N_B, N_ab, XsAFrameA[clusA %in% clustersA[-clustersA[j]],], XsBFrameA, XsAFrameB[clusA %in% clustersA[-clustersA[j]],], XsBFrameB, XsT[clusA %in% clustersA[-clustersA[j]],], XA, XB, X, met)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:n_A){
					...
					if (!is.null(dim(drop(pi_A))))
						YcA[i,] <- CalSF(ysA[-i,], ysB, pi_A[-i,-i], pi_B, pik_ab_B[-i], pik_ba_A, domains_A[-i], domains_B, N_A, N_B, N_ab, XsAFrameA[-i,], XsBFrameA, XsAFrameB[-i,], XsBFrameB, XsT[-i,], XA, XB, X, met)[[2]][1,]
					else
						YcA[i,] <- CalSF(ysA[-i,], ysB, pi_A[-i], pi_B, pik_ab_B[-i], pik_ba_A, domains_A[-i], domains_B, N_A, N_B, N_ab, XsAFrameA[-i,], XsBFrameA, XsAFrameB[-i,], XsBFrameB, XsT[-i,], XA, XB, X, met)[[2]][1,]	
				}
	...
	if (sdB == "str"){
		...
		for (i in 1:strataB){
			...
			for (j in (cnhB[i]+1):cnhB[i+1]){
				...
				if (!is.null(dim(drop(pi_B))))
					YcB[k,] <- CalSF(ysA, ysB[-j,], pi_A, pi_B[-j,-j], pik_ab_B, pik_ba_A[-j], domains_A, domains_B[-j], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-j,], XsAFrameB, XsBFrameB[-j,], XsT[-j,], XA, XB, X, met)[[2]][1,]		
				else 
					YcB[k,] <- CalSF(ysA, ysB[-j,], pi_A, pi_B[-j], pik_ab_B, pik_ba_A[-j], domains_A, domains_B[-j], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-j,], XsAFrameB, XsBFrameB[-j,], XsT[-j,], XA, XB, X, met)[[2]][1,]
				k <- k + 1
			}
		...
		if (sdB == "clu"){
			...
			for (i in 1:nclustersB){
				if (!is.null(dim(drop(pi_B))))
					YcB[i,] <- CalSF(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[i]],clusB %in% clustersB[-clustersB[i]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[i]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[i]],], XsT[clusB %in% clustersB[-clustersB[i]],], XA, XB, X, met)[[2]][1,]
				else
					YcB[i,] <- CalSF(ysA, ysB[clusB %in% clustersB[-clustersB[i]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[i]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[i]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[i]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[i]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[i]],], XsT[clusB %in% clustersB[-clustersB[i]],], XA, XB, X, met)[[2]][1,]
			}
			...
			if(sdB == "strclu"){
				...
				for (i in 1:nstrataB){
					...
					for (j in 1:nclustersB){
						...
						if (!is.null(dim(drop(pi_B))))
							YcB[k,] <- CalSF(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[j]],clusB %in% clustersB[-clustersB[j]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[j]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[j]],], XsT[clusB %in% clustersB[-clustersB[j]],], XA, XB, X, met)[[2]][1,]
						else
							YcB[k,] <- CalSF(ysA, ysB[clusB %in% clustersB[-clustersB[j]],], pi_A, pi_B[clusB %in% clustersB[-clustersB[j]]], pik_ab_B, pik_ba_A[clusB %in% clustersB[-clustersB[j]]], domains_A, domains_B[clusB %in% clustersB[-clustersB[j]]], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[clusB %in% clustersB[-clustersB[j]],], XsAFrameB, XsBFrameB[clusB %in% clustersB[-clustersB[j]],], XsT[clusB %in% clustersB[-clustersB[j]],], XA, XB, X, met)[[2]][1,]
						k <- k + 1
					}
				...
				for (i in 1:n_B){
	        			...
	        			if (!is.null(dim(drop(pi_B))))
						YcB[i,] <- CalSF(ysA, ysB[-i,], pi_A, pi_B[-i,-i], pik_ab_B, pik_ba_A[-i], domains_A, domains_B[-i], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-i,], XsAFrameB, XsBFrameB[-i,], XsT[-i,], XA, XB, X, met)[[2]][1,]
					else
						YcB[i,] <- CalSF(ysA, ysB[-i,], pi_A, pi_B[-i], pik_ab_B, pik_ba_A[-i], domains_A, domains_B[-i], N_A, N_B, N_ab, XsAFrameA, XsBFrameA[-i,], XsAFrameB, XsBFrameB[-i,], XsT[-i,], XA, XB, X, met)[[2]][1,]
				}
